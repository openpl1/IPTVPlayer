...
def getinfo():
    ...
    info_['icon'] = 'https://i.ibb.co/kc7KTfQ/egydead.png'
...
class TSIPHost(TSCBaseHostClass):
    ...
    def SearchResult(self, str_ch, page, extra):
        ...
        self.add_menu({'import': extra, 'url': url}, '', 'class="movieItem">.*?href="(.*?)".*?title="(.*?)".*?src="(.*?)"(.*?)</li>', '',
                      [('', 'video', ''), ('/assembly/', '20', 'URL'), ('/season/', '20', 'URL'), ('/serie/', '20', 'URL')], ord=[0, 1, 2, 3], Desc=desc, EPG=True)

    def getArticle(self, cItem):
        Desc = [('Genre', '<span>النوع : </span>(.*?)</li>', '', ''), ('Quality', '<span>الجوده : </span>(.*?)</li>', '', ''), ('Country', '<span>البلد : </span>(.*?)</li>', '', ''), ('Year', '<span>السنه : </span>(.*?)</li>', '', ''),
                ('Date', '<span>تاريخ الاصدار : </span>(.*?)</li>', '', ''), ('Duration', '<span>مده العرض : </span>(.*?)</li>', '', ''), ('Story', '<span>القصه</span>(.*?)</div>', '\n', '')]
        desc = ''
