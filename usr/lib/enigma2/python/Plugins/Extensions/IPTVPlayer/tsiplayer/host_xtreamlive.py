...
def getinfo():
    info_ = {}
    info_['name'] = 'Xtream IPTV (' + tscolor('\c0000????') + 'LIVE' + tscolor('\c00??????') + ')'
    info_['version'] = '2.1 17/11/2023'
    info_['dev'] = 'RGYSoft'
    info_['cat_id'] = '10'
    info_['desc'] = 'مشاهدة القنوات مباشر من اشتراكات xtream'
    info_['icon'] = 'https://i.ibb.co/SyTgjrC/Xtream.png'
...
class TSIPHost(TSCBaseHostClass):
    ...
    def showchannels(self, cItem):
        ...
        for elm in data:
            ...
            if cItem['xua'] != '':
                Url = strwithmeta(Url, {'User-Agent': cItem['xua']})
            if elm['stream_icon']:
                stream_icon = elm['stream_icon']
            else:
                stream_icon = ''
            if '---' in elm['name']:
                ...
                opt = False
                if exists('/etc/enigma2/ipaudio.json'):
                    opt = True
                if exists('/etc/enigma2/IPAudioPro.json'):
                    opt = True
                if exists('/etc/enigma2/ipaudioplus_user_list.json'):
                    opt = True
    ...
    def showoptions(self, cItem):
        ...
        if exists('/etc/enigma2/IPAudioPro.json'):
            self.addDir({'import': cItem['import'], 'category': 'host2', 'url': cItem['url'], 'title': 'Add to IPAUDIOPro', 'titre': cItem['title'], 'icon': cItem['icon'], 'mode': '26'})
        if exists('/etc/enigma2/ipaudioplus_user_list.json'):
            self.addDir({'import': cItem['import'], 'category': 'host2', 'url': cItem['url'], 'title': 'Add to IPAUDIOPlus', 'titre': cItem['title'], 'icon': cItem['icon'], 'mode': '27'})
        if exists('/etc/enigma2/iptosat.json'):
            self.addDir({'import': cItem['import'], 'category': 'host2', 'url': cItem['url'], 'title': 'Add to IPTOSAT', 'titre': cItem['title'], 'icon': cItem['icon'], 'mode': '25'})
    ...
    def addtoipaudiopro(self, cItem):
        try:
            elm = {"name": cItem['titre'], "display_name": cItem['titre'], "url": cItem['url']}
            with open('/etc/enigma2/IPAudioPro.json', 'r')as f:
                streams = json.loads(f.read())
            streams['streams'].append(elm)
            with open('/etc/enigma2/IPAudioPro.json', 'w') as f:
    ...
    def addtoipaudioplus(self, cItem):
        try:
            elm = {"Name": cItem['titre'], "Url": cItem['url']}
            with open('/etc/enigma2/ipaudioplus_user_list.json', 'r')as f:
                playlist = json.loads(f.read())
                elm.update({"Id": str(int(playlist['User List']["channels"][-1]['Id'])+1)})
            playlist['User List']["channels"].append(elm)
            with open('/etc/enigma2/ipaudioplus_user_list.json', 'w') as f:
                json.dump(playlist, f, indent=4)
            self.addMarker({'title': tscolor('\c00????00')+'#'+cItem['titre']+'# Successfully added', 'icon': cItem['icon']})
        except Exception as e:
            self.addMarker({'title': tscolor('\c00??0000')+'#'+cItem['titre']+'# Not added (Error)', 'icon': cItem['icon'], 'desc': str(e)})
    ...
    def start(self, cItem):
        ...
        if mode == '20':
            ...
            self.addtoipaudiopro(cItem)
        elif mode == '27':
            self.addtoipaudioplus(cItem)
        return True
