...
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.matrix.resources.lib.gui.hoster import \
    cHosterGui
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.matrix.resources.lib.home import \
    cHome
...
def getinfo():
    info_={}
    info_['name']='Matrix'
    info_['version']='2.1 03/07/2023'
    info_['dev']='RGYSOFT'
    info_['cat_id']='902'
    info_['desc']='Matrix (KODI Addon)'
    info_['icon']='https://i.ibb.co/xgxp4Ng/matrix.png'
    return info_
...
class TSIPHost(TSCBaseHostClass):
    ...
    def showResult(self,cItem):
        ...
        for Items in listing:
            ...
            if language.getActiveLanguage().split('_')[0] == 'en':
                sPack = ["Tools","My Accounts","Continue Watching","Search History","My Bookmarks"]
            elif language.getActiveLanguage().split('_')[0] == 'fr':
                sPack = ["Outils","Mes comptes","En cours de visionnage","Historique de recherche","Marque-pages"]
            elif language.getActiveLanguage().split('_')[0] == 'ar':
                sPack = ["أدوات","حساباتي","مواصلة المشاهدة","سجل البحث","المفضلة"]
            if (sPack[0] != sTitle.strip()) and (sPack[1] != sTitle.strip()) and (sPack[2] != sTitle.strip()) and (sPack[3] != sTitle.strip()) and (sPack[4] != sTitle.strip()):
