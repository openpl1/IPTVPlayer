...
from Plugins.Extensions.IPTVPlayer.p2p3.UrlLib import (urllib_quote,
                                                       urllib_unquote)
from Plugins.Extensions.IPTVPlayer.tools.iptvtools import MergeDicts, printDBG
...
def getinfo():
    info_ = {}
    name = 'Akwam ('+tscolor('\c0000????')+'Old'+tscolor('\c00??????')+')'
    hst = tshost(name)
    if hst == '':
        hst = 'https://akw.to/old/'
    info_['host'] = hst
    info_['name'] = name
    ...
    info_['icon'] = 'https://i.ibb.co/vB9nWss/akoam.png'
        ...
        if sts:
            ...
            for (url1, image, name_eng, desc) in lst_data:
                if 'توضيح هام لمتابعي' not in name_eng:
                    ...
                    desc = desc0+desc
                    self.addDir({'import': cItem['import'], 'category': 'host2', 'title': name_eng, 'url': url1.strip(), 'desc': ph.clean_html(desc), 'icon': image, 'mode': '31', 'good_for_fav': True, 'EPG': True, 'hst': 'tshost'})
                    count = count+1
            if count > 38:
        ...
        if sts:
            try:
                ...
                if 'golink' in cookies_:
                    link_from_cookie = json_loads(urllib_unquote(cookies_['golink']))['route'].replace('download', 'watching')
    ...
    def getArticle(self, cItem):
        ...
        if sts:
            ...
            for (x1, x2) in lst_dat0:
                if 'المدة الزمنية' in x1:
                    otherInfo1['duration'] = x2
                if 'سنة الانتاج' in x1:
                    otherInfo1['year'] = x2
                if 'اللغة' in x1:
                    otherInfo1['language'] = x2
                if 'جودة الصورة' in x1:
                    otherInfo1['quality'] = x2
                if 'الترجمة' in x1:
                    otherInfo1['translation'] = x2
                if 'المصدر' in x1:
                    otherInfo1['station'] = x2
                if 'نوع الملف' in x1:
                    otherInfo1['type'] = x2
                if 'محتوى الفيلم' in x1:
                    otherInfo1['category'] = x2

            lst_dat0 = re.findall('imdb">.*?<span>(.*?)</', lst_dat[0], re.S)
            if lst_dat0:
                otherInfo1['imdb_rating'] = lst_dat0[0].replace('\n', '')
            ...
            for (x1, x2, x3) in lst_dat0:
                if 'بطولة' in x1:
                    otherInfo1['actors'] = ph.clean_html(x2)
                if 'ﺇﺧﺮاﺝ' in x1:
                    otherInfo1['director'] = ph.clean_html(x2)
                if 'ﺗﺄﻟﻴﻒ' in x1:
                    otherInfo1['writers'] = ph.clean_html(x2)
                if 'تصنيف' in x1:
                    otherInfo1['genre'] = ph.clean_html(x2)

            if lst_dat0:
                desc = ph.clean_html(lst_dat0[0][1])
            else:
                desc = cItem['desc']
