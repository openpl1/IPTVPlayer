...
def getinfo():
    ...
    info_['icon'] = 'https://i.ibb.co/0K6TfBL/animeup.png'
...
class TSIPHost(TSCBaseHostClass):
    ...
    def get_links(self, cItem):
        ...
        sts, data = self.getPage(Url)
        if sts:
            iframe = re.findall('default" href="(.*?)".*?>(.*?)<', data, re.S | re.IGNORECASE)
            for (url, titre) in iframe:
                urlTab.append({'name': titre, 'url': url, 'need_resolve': 1})
        return urlTab

    def getArticle(self, cItem):
        printDBG("Anim4up.getVideoLinks [%s]" % cItem)
        otherInfo1 = {}
        desc = cItem['desc']
        ...
        if sts:
            ...
            for (xx) in lst_dat:
                ...
                for (x1, x2) in lst_dat0:
                    if 'حلقات' in x1:
                        otherInfo1['episodes'] = ph.clean_html(x2.replace('الحلقة', ''))
                    if 'لمدة' in x1:
                        otherInfo1['duration'] = ph.clean_html(x2)
                    if 'صاحب' in x1:
                        otherInfo1['station'] = ph.clean_html(x2)
                    if 'لإسم العر' in x1:
                        otherInfo1['alternate_title'] = ph.clean_html(x2)
    ...
    def get_info(self, data):
        ...
        for (xx) in lst_dat:
            ...
            for (x1, x2) in lst_dat0:
                if 'حلقات' in x1:
                    inf = inf + tscolor('\c00????00') + 'Episodes: '+tscolor('\c00??????')+ph.clean_html(x2.replace('الحلقة', ''))+' | '
                if 'لمدة' in x1:
                    inf = inf + tscolor('\c00????00') + 'Duration: '+tscolor('\c00??????')+ph.clean_html(x2)+' | '
