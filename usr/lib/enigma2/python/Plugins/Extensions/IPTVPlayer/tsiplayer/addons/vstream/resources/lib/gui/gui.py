...
from Plugins.Extensions.IPTVPlayer.libs.xbmc import xbmc, xbmcplugin
...
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.comaddon import (
    VSlog, addon, dialog, isNexus, listitem, progress, window)
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.gui.contextElement import \
    cContextElement
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.gui.guiElement import \
    cGuiElement
...
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.parser import \
    cParser
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.tmdb import \
    cTMDb
...
class cGui:
    ...
    def viewBA(self):
        ...
        from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.ba import \
            cShowBA
        cBA = cShowBA()
    ...
    def viewInfo(self):
        if addon().getSetting('information-view') == "false":
            from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.config import \
                WindowsBoxes
    ...
    def viewParents(self):
        ...
        if 'movie'in sType:
            ...
            sUrl = 'https://www.imdb.com/title/'+sIMDb+'/parentalguide?ref_=tt_stry_pg'
        oRequest = urllib2_Request(sUrl)
        oResponse = urllib_urlopen(oRequest)
        DIALOG = dialog()
    ...
    def setWatched(self):
        if True:
            ...
            from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.db import \
                cDb
            with cDb() as db:
    ...
    def addSearchResult(self, oGuiElement, oOutputParameterHandler):
        ...
        cGui.searchResults[searchSiteId].append({'guiElement': oGuiElement, 'params': copy.deepcopy(oOutputParameterHandler)})
        cGui.searchResultsSemaphore.release()
