...
from Plugins.Extensions.IPTVPlayer.libs.xbmc import xbmc, xbmcgui
...
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.comaddon import (
    VSlog, addon, dialog, isMatrix, siteManager)
...
def getReso(sMovieTitle, pos):
    ...
    sPattern = ['HDCAM', '[^\w](CAM)[^\w]', '[^\w](R5)[^\w]', '.(3D)', '.(DVDSCR)', '.(TVRIP)', '.(FHD)', '.(HDLIGHT)', '.(4K)', '.(UHD)',
                '\d{3,4}P', '.(HDRIP)', '.(BDRIP)', '.(BRRIP)', '.(DVDRIP)', '.(HDTV)', '.(BLURAY)', '.(WEB-DL)', '.(WEBDL)', '.(WEBRIP)', '[^\w](WEB)[^\w]', '.(DVDRIP)']
...
def upToMyAccount():
    ...
    if aResult:
        ...
        req = urllib2_Request(upUrl, mpartdata[1], headers)
