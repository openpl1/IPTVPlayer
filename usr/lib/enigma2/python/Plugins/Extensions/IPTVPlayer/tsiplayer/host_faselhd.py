...
def getinfo():
    ...
    info_['icon'] = 'https://i.ibb.co/R30VkGN/faselhd.png'
...
class TSIPHost(TSCBaseHostClass):
    ...
    def showsearch(self, cItem):
        ...
        self.addDir({'import': cItem['import'], 'category': 'host2', 'title': 'البحث عن انمي', 'icon': cItem['icon'], 'mode': '51', 'section': 'انمي'})
    ...
    def getPage0(self, baseUrl, addParams={}, post_data=None):
        ...
        if addParams == {}:
            addParams = dict(self.defaultParams0)
    ...
    def showelms(self, cItem):
        ...
        if sts:
            ...
            if lst_data:
                self.addVideo({'import': cItem['import'], 'good_for_fav': True, 'name': 'categories', 'category': 'video', 'url': lst_data[0], 'title': 'Trailer', 'desc': '', 'icon': cItem['icon'], 'hst': 'none'})
            ...
            if Liste_els:
                ...
                for (url, titre) in Liste_els:
                    ...
                    if ('سيرفر #01' in titre) or ('سيرفر الجودة الأصلية' in titre):
                        ...
                        local = 'local'
                    elif 'سيرفر #07' in titre:
                        titre = '|Server 07| Vidfast.Co'
                    elif 'سيرفر المشاهدة #01' in titre:
                        titre = '|Server 01| FaselHd'
                    elif 'سيرفر المشاهدة #02' in titre:
                        titre = '|Server 02| FaselHd'
    ...
    def getArticle(self, cItem):
        ...
        if sts:
            ...
            if Liste_els_3:
                desc = self.cleanHtmlStr(Liste_els_3[0])
