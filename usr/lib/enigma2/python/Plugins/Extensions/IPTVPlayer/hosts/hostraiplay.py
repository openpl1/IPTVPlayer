            ...
            if item["sub-type"] in ("RaiPlay Tipologia Page", "RaiPlay Genere Page", "RaiPlay Tipologia Editoriale Page" ):
                ...
                if not (item["name"] in ("Teatro", "Musica")):
                    ...
                    if m:
                        ...
                        if m[0]=="fiction":
                            params = MergeDicts(cItem, {'category':'ondemand_items', 'title': "Serie italiane", 'name': "Serie italiane", 'url': "/raiplay/tipologia/serieitaliane/index.json", 'icon': "http://www.rai.it/dl/img/2018/06/04/1528107006058_ico-fiction.png", 'sub-type': item["sub-type"] })
                            printDBG(str(params))
                            self.addDir(params)

                            params = MergeDicts(cItem, {'category':'ondemand_subhome', 'title': "Original", 'name': "Original", 'url': "/raiplay/tipologia/original/index.json", 'icon': "http://www.rai.it/dl/img/2018/06/04/1528107006058_ico-fiction.png", 'sub-type': item["sub-type"] })
                            printDBG(str(params))
                            self.addDir(params)

                        elif m[0]=="serietv":

                            params = MergeDicts(cItem, {'category':'ondemand_items', 'title': "Serie internazionali", 'name': "Serie internazionali", 'url': "/raiplay/tipologia/serieinternazionali/index.json", 'icon': "http://www.rai.it/dl/img/2018/06/04/1528107006058_ico-fiction.png", 'sub-type': item["sub-type"] })
                            printDBG(str(params))
                            self.addDir(params)

                        elif m[0]=="bambini" or m[0]=="bambini/":
                            if "image" in item:
                                icon = self.getThumbnailUrl(item["image"])
