...
class UpdateMainAppImpl(IUpdateObjectInterface):
    ...
    def __addLastVersion(self, servers):
        ...
        sts, data = self.cm.getPage(mainUrl + '/tree/main')
        if sts:
        ...
        if DMHelper.STS.DOWNLOADED != status:
            ...
            if os_path.isfile(newVerFile):
                try:
                    ...
                    printExc()
                if 13 == len(newVerNum):
                    sourceUrl = "https://gitlab.com/{0}/e2iplayer/-/archive/main/e2iplayer-main.tar.gz".format(config.plugins.iptvplayer.gitlab_repo.value)
