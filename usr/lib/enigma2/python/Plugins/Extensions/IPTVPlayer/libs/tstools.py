...
from Plugins.Extensions.IPTVPlayer.tools.iptvtools import (
    CSearchHistoryHelper, GetCacheSubDir, GetCookieDir, printDBG, printExc)
...
class URLResolver():
    ...
    def getLinks(self):
        ...
        if e2_parse.checkHostSupport(self.sHosterUrl)==1:
            urlTab = e2_parse.getVideoLinkExt(self.sHosterUrl)
        else:
            printDBG('------------> No parse Find <-------------')
            urlTab = e2_parse.getVideoLinkExt(self.sHosterUrl)
        return urlTab
...
def decal(tab,step,step2,decal_fnc):
    ...
    while True:
        ...
        del tab[0]
        exec(decal_fnc)
        if ((step2 == step) or (aa>10000)): break
...
def resolve_zony(link,referer):
    URL=''
    cm = common()
    USER_AGENT = 'Mozilla/5.0 (Android 13; Mobile; rv:108.0) Gecko/108.0 Firefox/108.0'
    HEADER = {'User-Agent': USER_AGENT, 'Connection': 'keep-alive', 'Accept-Encoding':'gzip', 'Content-Type':'application/x-www-form-urlencoded','Referer':''}
    defaultParams = {'header':HEADER, 'use_cookie': False, 'load_cookie':  False, 'save_cookie': False}
    urlo = link.replace('embedplayer','membedplayer')
    params = dict(defaultParams)
...
class TSCBaseHostClass:
    ...
    def getData(self, DataUrl,):
        DataUrl = DataUrl.encode('latin-1').decode('utf-8')
        if 'adilbo' in DataUrl:
            t_script = re.findall('<script.*?;.*?\'(.*?);', DataUrl, re.S)
            t_int = re.findall('/g.....(.*?)\)', DataUrl, re.S)
            if t_script and t_int:
                ...
                if isPY2():
                    ...
                    return page.encode('latin-1').decode('utf-8')
        else:
            return ''
    ...
    def std_extract(self,titre):
        target = "الحلقة الأخيرة"
        sTitel = self.cleanHtmlStr(titre)
        if target in sTitel:
            episode = target
        else:
    ...
    def MediaBoxResult(self,str_ch,year_,extra):
        urltab=[]
        str_ch_o = str_ch
        str_ch = urllib_quote(str_ch_o+' '+year_)
        result = self.SearchResult(str_ch,1,'')
        if result ==[]:
            str_ch = urllib_quote(str_ch_o)
            result = self.SearchResult(str_ch,1,'')
        for elm in result:
            titre     = elm['title']
            url       = elm['url']
