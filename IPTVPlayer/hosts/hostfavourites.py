...
class Favourites(CBaseHostClass):
    ...
    def listFavourites(self, cItem):
        ...
        for idx in range(len(data)):
            ...
            if "tsiplayer" in item.hostName:
                HostName = search(r"host_(\w+)",item.data).group(1)
                if HostName == 'matrix':
                    match = search(r'"sSiteName":\s*"([^"]+)"', item.data).group(1)
                    desc = _('\c00????00 Source\c00??????: Matrix_{}\n{}'.format(match.capitalize(),item.description))
                else:
                    desc = _('\c00????00 Source\c00??????: {}\n{}'.format(HostName.capitalize(),item.description))
