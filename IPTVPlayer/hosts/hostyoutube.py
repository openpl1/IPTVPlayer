...
from Components.config import ConfigDirectory, config, getConfigListEntry
...
from Plugins.Extensions.IPTVPlayer.tools.iptvtools import (GetSearchHistoryDir,
                                                           IsExecutable,
                                                           byteify, printDBG,
                                                           printExc)
...
class Youtube(CBaseHostClass):
    ...
    def __init__(self):
        ...
        self.CheckLine = self.LenHistory()
        ...
        self.MAIN_GROUPED_TAB = [{'category': 'search', 'title': _("Search"), 'desc': _("Search youtube materials "), 'search_item': True},
                                {'category': 'from_file', 'title': _("User links"), 'desc': _("User links stored in the ytlist.txt file.")},
        ...
        if self.CheckLine != 0:
            self.MAIN_GROUPED_TAB.append({'category': 'delete_history','title': _('Delete search history'),'desc':self.CheckLine })
    ...
    def listFeeds(self, cItem):
        ...
        if cItem['category'] == "feeds_video":
            ...
            self.addDir(params)
            title = _("Music")
            url = "https://www.youtube.com/feed/trending?bp=4gINGgt5dG1hX2NoYXJ0cw=="
            params = {'category': 'feeds_video', 'title': title, 'url': url}
            self.addDir(params)
            title = _("Games")
            url = "https://www.youtube.com/feed/trending?bp=4gIcGhpnYW1pbmdfY29ycHVzX21vc3RfcG9wdWxhcg=="
            params = {'category': 'feeds_video', 'title': title, 'url': url}
            self.addDir(params)
            title = _("Movies")
            url = "https://www.youtube.com/feed/trending?bp=4gIKGgh0cmFpbGVycw=="
            params = {'category': 'feeds_video', 'title': title, 'url': url}
            self.addDir(params)
            title = _("Live")
    ...
    def delhistory(self):
        ...
        ret = self.sessionEx.waitForFinishOpen(MessageBox, 'Are you sure you want to delete search history?', type=MessageBox.TYPE_YESNO, default=True)
        if ret[0]:
            open(GetSearchHistoryDir(self.UTLIST_FILE), 'w').close()
            self.sessionEx.waitForFinishOpen(MessageBox, 'Search History successfully deleted.', type=MessageBox.TYPE_INFO,timeout=5)
            self.listMainMenu()

    def LenHistory(self):
        if not exists(GetSearchHistoryDir(self.UTLIST_FILE)):
            open(GetSearchHistoryDir(self.UTLIST_FILE), 'w').close()

        with open(GetSearchHistoryDir(self.UTLIST_FILE), 'r',) as file:
            line = file.readlines()
        if len(line) == 0:
            return len(line)
        else:
            return ("Number of items in search history: " + str(len(line)))
