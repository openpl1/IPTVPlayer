...
def getinfo():
    ...
    info_['icon'] = 'https://i.ibb.co/hy6T62x/wecima.png'
...
class TSIPHost(TSCBaseHostClass):
    def __init__(self):
        ...
        self.HEADER = {'User-Agent': self.USER_AGENT, 'Connection': 'keep-alive', 'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8',
                       'Content-Type': 'application/x-www-form-urlencoded', 'Referer': self.getMainUrl(), 'Origin': self.getMainUrl()}
    ...
    def showsearch(self, cItem):
        ...
        self.addDir({'import': cItem['import'], 'category': 'host2', 'title': 'البحث عن فيلم', 'icon': cItem['icon'], 'mode': '51', 'section': 'films'})
        self.addDir({'import': cItem['import'], 'category': 'host2', 'title': 'البحث عن مسلسل', 'icon': cItem['icon'], 'mode': '51', 'section': 'series'})
        self.addDir({'import': cItem['import'], 'category': 'host2', 'title': 'البحث في انمي', 'icon': cItem['icon'], 'mode': '51', 'section': 'anime'})
    ...
    def showitms(self, cItem):
        if '/Filtering/genre/' in cItem['url']:
            ...
            page = cItem.get('page', 1)
            if page != 1:
                URL = cItem['url'] + '/offset/'+str((page-1)*30)+'/'
            ...
            self.add_menu(cItem, '<div class="Grid--WecimaPosts">(.*?)(?:class="pagination">|class="RightUI">)',
                          'class="GridItem".*?href="(.*?)".*?image:url\((.*?)\).*?</span>(.*?)</strong>(.*?)</ul>', '', '21', Next=Next, ord=[0, 2, 1, 3], u_titre=True, EPG=True)
    ...
    def SearchResult(self, str_ch, page, extra):
        ...
        self.add_menu({'import': extra, 'url': url}, '<div class="Grid--WecimaPosts">(.*?)(?:class="pagination">|class="RightUI">)',
                      'class="GridItem".*?href="(.*?)".*?image:url\((.*?)\).*?</span>(.*?)</strong>(.*?)</ul>', '', '21', ord=[0, 2, 1, 3], u_titre=True, EPG=True)
    ...
    def getVideos(self, videoUrl):
        ...
        if result[1] == []:
            result = self.add_menu({'url': videoUrl}, '', 'source.*?(src)="(.*?)"', result[0], 'link4', ord=[1, 0])
        return result[1]
