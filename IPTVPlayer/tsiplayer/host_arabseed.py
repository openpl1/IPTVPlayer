...
def getinfo():
    ...
    info_['icon'] = 'https://i.ibb.co/WtH2Wp2/arabseed.png'
...
class TSIPHost(TSCBaseHostClass):
    def __init__(self):
        ...
        self.MAIN_URL = requests.get(getinfo()['host'], allow_redirects=True).url
    ...
    def showmenu1(self, cItem):
        ...
        if gnr == 2:
            self.addDir({'import': cItem.get('import', ''), 'title': 'Ramadan 2022', 'category': 'host2', 'url': self.MAIN_URL +
                        '/category/%d8%a7%d9%84%d9%85%d8%b3%d9%84%d8%b3%d9%84%d8%a7%d8%aa/%d9%85%d8%b3%d9%84%d8%b3%d9%84%d8%a7%d8%aa-%d8%b1%d9%85%d8%b6%d8%a7%d9%86-2022', 'desc': '', 'icon': cItem.get('icon', ''), 'mode': '20'})
        ...
        if data1:
            ...
            for (x1, x2, x3) in lst_data1:
                if not ((x1 == 'category') and (x2.strip() == '')):
                    ...
                    code = codeold
                self.addDir({'import': cItem.get('import', ''), 'category': 'host2', 'url': code, 'title': ph.clean_html(x3), 'desc': code,
                            'icon': cItem.get('icon', ''), 'mode': mode_, 'count': count+1, 'data': data1, 'code': code, 'sub_mode': 'item_filter', 'page': -1})
    ...
    def showitms(self, cItem):
        ...
        if cItem['url'].startswith('http') or (cItem['url'].startswith('/')):
            ...
            self.add_menu(cItem, 'Blocks">(.*?)</ul>', 'class="MovieBlock.*?href="(.*?)".*?(?:image=|img src=)"(.*?)"(.*?)<h4>(.*?)</h4>(.*?)</a>',
                          '', '21', LINK=LINK, post_data=post_data, ind_0=-1, ord=[0, 3, 1, 2, 4], Desc=desc, Next=next, u_titre=True, EPG=True)
        ...
        if sts:
            ...
            if lst_dat:
                ...
                for (x1, x2) in lst_dat0:
                    if 'الجودة' in x1:
                        otherInfo1['quality'] = ph.clean_html(x2)
                    if 'تاريخ' in x1:
                        otherInfo1['year'] = ph.clean_html(x2)
                    if 'اللغة' in x1:
                        otherInfo1['language'] = ph.clean_html(x2)
                    if 'النوع' in x1:
                        otherInfo1['genres'] = ph.clean_html(x2)
                    if 'الدولة' in x1:
                        otherInfo1['country'] = ph.clean_html(x2)
                    if 'السنه' in x1:
                        otherInfo1['year'] = ph.clean_html(x2)
