...
def getinfo():
    ...
    info_['icon'] = 'https://i.ibb.co/nBz6Rfz/esseq.png'
...
class TSIPHost(TSCBaseHostClass):
    ...
    def get_links(self, cItem):
        ...
        sts, data = self.getPage(baseUrl, param)
        if sts:
            Srct = re.findall('class="skipAd">.+?<span>.+?href="(.+?)">', data, re.M | re.I | re.DOTALL)
            sts, data = self.getPage(Srct[0], param)
            ...
            if lst_data:
                for host, url in lst_data:
                    if '<em>ok</em>' in url:
                        ...
                        'pro'
                        url = 'https://segavid.com/embed-'+host+'.html'
                        host = 'PROTONVIDEO'
                ...
                if lst_data:
                    urlTab.append({'url': lst_data[0], 'name': 'Iframe', 'need_resolve': 1, type: ''})
        return urlTab
