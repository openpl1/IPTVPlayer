...
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.gui.hoster import \
    cHosterGui
from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.lib.home import \
    cHome
...
def getinfo():
    ...
    info_['icon']='https://i.ibb.co/cDz1XLH/vstream.png'
    return info_
...
def getHosts():
    ...
    for (id_,titre,hst_,version_,desc_,up_,image) in Hosts:
        if image=='': image = 'file:///usr/lib/enigma2/python/Plugins/Extensions/IPTVPlayer/tsiplayer/addons/vstream/resources/art/sites/'+hst_+'.png'
        if titre=='': titre = hst_.replace('_','.').title()
        imp0  = 'from Plugins.Extensions.IPTVPlayer.tsiplayer.addons.vstream.resources.sites.'
