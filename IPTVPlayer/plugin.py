...
from Plugins.Extensions.IPTVPlayer.components.iptvplayerinit import \
    TranslateTXT as _
from Plugins.Extensions.IPTVPlayer.components.iptvplayerwidget import \
    E2iPlayerWidget
...
from Plugins.Extensions.IPTVPlayer.tools.iptvtools import (
    IsExecutable, IsWebInterfaceModuleAvailable, printDBG)
...
def runMain(session, nextFunction=doRunMain):
    def allToolsFromOPKG():
        toolsList = ['enigma2-plugin-extensions-e2iplayer-deps', 'duktape', 'exteplayer3', 'uchardet', 'gstplayer', 'rtmpdump', 'python3-e2icjson', 'python3-pycurl', 'python3-sqlite3']
        if isPY2():
            toolsList = list(map(lambda x: x.replace('python3', 'python'), toolsList))
        for tool in toolsList:
            ...
            if os.path.exists('/etc/opkg/opkg.conf'):
                ...
                if not os.path.exists(os.path.join('/var/lib/dpkg/info/', tool) + '.control'):
                    tryToInstall += ' ' + tool
        if tryToInstall != '':
            if os.path.exists('/etc/opkg/opkg.conf'):
                ...
                printDBG('allToolsFromDPKG() >>> Trying to install missing packages: %s' % tryToInstall)
                os.system('(apt-get update;apt-get install -y %s)&' % tryToInstall)
                return False
        else:
